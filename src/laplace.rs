...
impl Laplace{
    ...
    pub fn laplace2d(p:&mut Array2<f64>,dx:f64,dy:f64,l1norm_target:f64){
        let mut l1norm =1.;
        let row = p.shape()[0];
        let col = p.shape()[1];
        ...
        while l1norm>l1norm_target{
            ...
            Zip::from(p.slice_mut(s![..,col-1]))
                ...
                .apply(|pb,&b|*pb=b);
            l1norm= (Self::abs(&p)-Self::abs(&pn)).sum()/pn.sum();
        }
    }
    ...
    pub fn compute(){
        ...
        Zip::from(p.slice_mut(s![..,col-1]))
            .and(pn.slice(s![..,col-2]))
            .apply(|pb,&b|*pb=b);
        Self::plot2D(&p, nx, ny, 2., 2.);
        Self::laplace2d(&mut p, dx, dy, 1e-4);
        Self::plot2D(&p, nx, ny, 2., 2.);
    }
}
